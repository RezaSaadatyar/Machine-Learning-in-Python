...
def roc_curve(model, data_train, data_test, label_train, label_test, j, fig_size=(5, 3), type_class=" ", display_class="off"):
   ...
   num_classes = np.max(label_train) + 1            # Binarize the labels for each class
   ...
   # -------------------------------------------------- ROC curve for each class --------------------------------------------------
   if display_class == "on":
      fig, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, figsize=fig_size, constrained_layout=True)
      for i in range(label_tr.shape[1]):
      ...
      ax1.legend(title="AUC", loc='lower right',fontsize=9, ncol=1, frameon=False, labelcolor='linecolor', handlelength=0)
      ...
      ax2.legend(title="AUC", loc='lower right',fontsize=9, ncol=1, frameon=False, labelcolor='linecolor', handlelength=0)
     
      plt.autoscale(axis="x", tight=True, enable=True)
      fig.suptitle(f"{type_class} ROC Curve for a {j+1}-fold cross-validation", fontsize=10)
